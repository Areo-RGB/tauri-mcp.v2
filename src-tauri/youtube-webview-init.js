(() => {
  ...
  let videoId = '';
  const icon = `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 3v12m0 0 5-5m-5 5-5-5M5 20h14"/></svg>`;
  ...
  async function process(list,button) {
    ...
    const old=button?.innerHTML;if(button){button.disabled=true;button.textContent=`Processing ${list.length}…`}
    ...
    } catch(error){note.textContent=`Chapter Clipper: ${error}`;setTimeout(()=>note.remove(),10000)} finally{if(button){button.disabled=false;button.innerHTML=old}}
  }
  function inject() {
    ...
    for(const chapter of list){const row=chapter.item.querySelector('#endpoint,a')||chapter.item;
      ...
      if(!chapter.item.querySelector('.mh-row')){const button=document.createElement('button');button.className='mh-row';button.title='Download this chapter';button.setAttribute('aria-label',`Download ${chapter.title}`);button.innerHTML=icon;button.onclick=e=>{e.preventDefault();e.stopPropagation();process([chapter],button)};row.append(button)}
    }
    ...
    const button=document.createElement('button');button.className='mh-download';button.innerHTML=`${icon}<span>Download</span>`;button.title='Download selected chapters';button.onclick=()=>process(chapters().filter(c=>selected.has(c.startTime)),button);transcript?transcript.insertAdjacentElement('afterend',button):anchor.append(button);
  }
  function mount() {
